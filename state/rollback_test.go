...
func TestRollback(t *testing.T) {
	...
	nextState.NextValidators = initialState.NextValidators.CopyIncrementProposerPriority(1, 0)
...
func TestRollbackHard(t *testing.T) {
	...
	block := &types.Block{
		Header: types.Header{
			...
			NextValidatorsHash: valSet.CopyIncrementProposerPriority(1, 0).Hash(),
	...
	currState := state.State{
		...
		Validators:                       valSet.CopyIncrementProposerPriority(1, 0),
		NextValidators:                   valSet.CopyIncrementProposerPriority(2, 0),
	...
	nextBlock := &types.Block{
		Header: types.Header{
			...
			LastCommitHash:     crypto.CRandBytes(tmhash.Size),
			DataHash:           crypto.CRandBytes(tmhash.Size),
			ValidatorsHash:     valSet.CopyIncrementProposerPriority(1, 0).Hash(),
			NextValidatorsHash: valSet.CopyIncrementProposerPriority(2, 0).Hash(),
			ConsensusHash:      params.Hash(),
	...
	nextState := state.State{
		...
		AppHash:                          crypto.CRandBytes(tmhash.Size),
		LastValidators:                   valSet.CopyIncrementProposerPriority(1, 0),
		Validators:                       valSet.CopyIncrementProposerPriority(2, 0),
		NextValidators:                   valSet.CopyIncrementProposerPriority(3, 0),
		ConsensusParams:                  *params,
