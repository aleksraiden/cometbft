...
func NewGoLevelDB(name string, dir string) (*GoLevelDB, error) {
	return NewGoLevelDBWithOpts(name, dir, DefaultGoLevelDBOptions())
}

func DefaultGoLevelDBOptions() *opt.Options {
	return &opt.Options{
		BlockCacheCapacity:            opt.DefaultBlockCacheCapacity,
		OpenFilesCacheCapacity:        opt.DefaultOpenFilesCacheCapacity,
		BlockSize:                     opt.DefaultBlockSize,
		CompactionL0Trigger:           opt.DefaultCompactionL0Trigger,
		CompactionTableSize:           opt.DefaultCompactionTableSize,
		CompactionTotalSize:           opt.DefaultCompactionTotalSize,
		WriteBuffer:                   opt.DefaultWriteBuffer,
		WriteL0SlowdownTrigger:        opt.DefaultWriteL0SlowdownTrigger,
		WriteL0PauseTrigger:           opt.DefaultWriteL0PauseTrigger,
		IteratorSamplingRate:          opt.DefaultIteratorSamplingRate,
		Compression:                   opt.DefaultCompressionType,
		CompactionTableSizeMultiplier: opt.DefaultCompactionTableSizeMultiplier,
		CompactionTotalSizeMultiplier: opt.DefaultCompactionTotalSizeMultiplier,
	}
}
...
func (db *GoLevelDB) Stats() map[string]string {
	...
	for level := 0; level < 7; level++ {
		key := "leveldb.num-files-at-level" + strconv.Itoa(level)
		str, err := db.db.GetProperty(key)
		if err == nil {
			stats[key] = str
		}
